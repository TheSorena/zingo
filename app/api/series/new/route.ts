  ...
  try {
    ...
    if (page > 0) {
      const data = await fetchUpstreamJson(
        `${apiUrl}/api/serie/by/filtres/0/created/${page}/4F5A9C3D9A86FA54EACEDDD635185/`
      );
    ...
    while (allSeries.length < limit) {
      try {
        const pageData = await fetchUpstreamJson<any>(
          `${apiUrl}/api/serie/by/filtres/0/created/${currentPage}/4F5A9C3D9A86FA54EACEDDD635185/`
        );

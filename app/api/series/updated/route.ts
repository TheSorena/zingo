  ...
  try {
    ...
    if (page > 0) {
      const response = await fetch(
        `${apiUrl}/api/poster/by/filtres/31/0/created/${page}/4F5A9C3D9A86FA54EACEDDD635185/`,
        {
          headers: {
    ...
    while (allSeries.length < limit) {
      const response = await fetch(
        `${apiUrl}/api/poster/by/filtres/31/0/created/${currentPage}/4F5A9C3D9A86FA54EACEDDD635185/`,
        {
          headers: {
            'Accept': 'application/json'
          },

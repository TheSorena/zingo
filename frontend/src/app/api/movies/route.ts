...
async function fetchFromApi(path: string): Promise<any[]> {
  try {
    const res = await fetch(`${API_BASE}${path}${path.endsWith('/') ? '' : '/'}${API_KEY}/`, {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      ...
      items = await fetchFromApi(`/api/movie/by/filtres/0/imdb/${page}`);
    } else {
      items = await fetchFromApi(`/api/movie/by/filtres/0/created/${page}`);
    }

    const movies = items.map(transformItem);
    return NextResponse.json({
      success: true,
      data: { movies, pagination: { page: parseInt(page), limit: 30, total: movies.length, totalPages: 999 } }
    });
  } catch (error) {
    console.error('Movies proxy error:', error);
    return NextResponse.json({ success: false, message: 'خطا در دریافت فیلم‌ها', data: { movies: [] } }, { status: 500 });
  }
}

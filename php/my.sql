...
-- Add foreign key constraints for campaigns table after all tables are created
ALTER TABLE `campaigns`
  ADD CONSTRAINT `campaigns_rss_automations_rss_automation_id_fk` FOREIGN KEY (`rss_automation_id`) REFERENCES `rss_automations` (`rss_automation_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `campaigns_recurring_campaigns_recurring_campaign_id_fk` FOREIGN KEY (`recurring_campaign_id`) REFERENCES `recurring_campaigns` (`recurring_campaign_id`) ON DELETE SET NULL ON UPDATE CASCADE;
